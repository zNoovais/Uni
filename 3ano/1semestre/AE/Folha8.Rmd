...
mdl <- glm(Survived ~ Pclass + Sex + Age + Fare, data = treino,family= binomial(link="logit"))
...
prob <- predict(mdl, newdata=teste ,type = "response")
head(prob)

table <- ifelse(prob >= 0.5, "TRUE", "FALSE")
a <- table(Real = teste$Survived, Predito = table)
...
roc_obj <- roc(teste$Survived, prob)
