...
corr <- prcomp(X,center = TRUE,scale=FALSE)
glimpse(corr)
summary(corr)
...
scores <- as.data.frame(corr$x)

scores$lab <- labs

plot(corr$x[,1], corr$x[,2], col = as.factor(NCI60$labs))
...
scores <- as.data.frame(corr$x)
scores$Tipo <- NCI60$labs   # adiciona as labels
...
ggplot(scores, aes(x = PC3, y = PC2, color = y)) +
  geom_point(size = 3, alpha = 0.8)
